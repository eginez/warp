...
use warpui::{
    Presenter, WindowInvalidation, notification::UserNotification, platform::WindowStyle,
};
...
use crate::pane_group::{BackingView, TerminalPaneId, pane::PaneStack};
...
#[test]
fn programmatic_pty_write_writes_bytes_without_touching_input_buffer() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
        let pty_writes: Rc<RefCell<Vec<Vec<u8>>>> = Rc::new(RefCell::new(Vec::new()));
        ...
        terminal.update(&mut app, |view, ctx| {
            view.input.update(ctx, |input, ctx| {
                input.system_insert("existing input", ctx);
            });

            view.write_programmatic_bytes_to_pty(b"pwd\r".to_vec(), ctx);
        });

        terminal.read(&app, |view, ctx| {
            assert_eq!(pty_writes.borrow().as_slice(), &[b"pwd\r".to_vec()]);
            assert_eq!(view.input.as_ref(ctx).buffer_text(ctx), "existing input");
        });
    })
}
